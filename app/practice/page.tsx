...
import { getSection, getAllSections } from '@/lib/db';
...
function PracticeSelectorContent() {
  ...
  useEffect(() => {
    if (sectionId) {
      ...
      // Load all sections for grid display
        ...
        {!sectionId ? (
          <section className="py-12 sm:py-16 md:py-24">
            ...
            <p className="text-center text-sm sm:text-base text-ink-navy font-body mb-8 sm:mb-12 opacity-80">
              Select which math level you want to practice
            </p>

            {loading ? (
              <div className="text-center py-12">
                <p className="text-ink-navy font-body">Loading sections...</p>
              </div>
            ) : (
              <SectionGrid sections={allSections} linkPrefix="/practice" />
            )}
          </section>
        ) : section ? (
